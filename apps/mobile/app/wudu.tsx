...
import { wuduSteps, WuduStep } from '../src/data/wuduSteps';
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: bgColor }]} edges={['top']}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <Text style={[styles.title, { color: textColor }]}>Guide Wudu</Text>
          <Text style={[styles.subtitle, { color: colors.gold }]}>الوضوء</Text>
          <Text style={[styles.description, { color: textSecondary }]}>
            L'ablution (wudu) est un acte de purification obligatoire avant la prière.
          </Text>
        </View>
        ...
        <View style={[styles.progressContainer, { backgroundColor: cardBg }, shadows.sm]}>
          <View style={styles.progressInfo}>
            <Text style={[styles.progressText, { color: textColor }]}>
              {completedSteps.length} / {wuduSteps.length} étapes
            </Text>
            <Text style={[styles.progressPercent, { color: colors.gold }]}>
              {Math.round((completedSteps.length / wuduSteps.length) * 100)}%
            </Text>
          </View>
          <View style={[styles.progressBar, { backgroundColor: textSecondary + '30' }]}>
            <View
              style={[
                styles.progressFill,
                {
                  width: `${(completedSteps.length / wuduSteps.length) * 100}%`,
        ...
        {wuduSteps.map((step) => {
          ...
          return (
            ...
            >
              ...
              <View style={[styles.sunnahContainer, { backgroundColor: colors.gold + '15' }]}>
                <Ionicons name="star" size={14} color={colors.gold} />
                <Text style={[styles.sunnahText, { color: colors.gold }]}>{step.sunnah}</Text>
              </View>
            </Touchable>
          );
        })}
...
const styles = StyleSheet.create({
  ...
  sunnahContainer: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs, paddingHorizontal: spacing.sm, paddingVertical: spacing.xs, borderRadius: borderRadius.sm, alignSelf: 'flex-start' },
  sunnahText: { fontSize: 12, fontWeight: '500' },
